...
function loadNativeModule() {
    ...
    // Also check for Neon-built module at native/index.node
    const neonBinaryPath = path.join(
        __dirname,
        '..',
        'native',
        'index.node'
    ...
    // Check if prebuilt binary exists, fall back to neon binary
    let activeBinaryPath = binaryPath;
    if (!fs.existsSync(binaryPath)) {
        if (fs.existsSync(neonBinaryPath)) {
            console.log(`📍 ZCashSkills: Using Neon-built binary at ${neonBinaryPath}`);
            activeBinaryPath = neonBinaryPath;
        } else {
    ...
    try {
        ...
        const nativeModule = require(activeBinaryPath);
        ...
        if (!nativeModule.scanNotes) {
            console.warn('⚠️  Native module missing scanNotes — send feature unavailable (rebuild required)');
        }
        if (!nativeModule.createTransaction) {
            console.warn('⚠️  Native module missing createTransaction — send feature unavailable (rebuild required)');
        }
